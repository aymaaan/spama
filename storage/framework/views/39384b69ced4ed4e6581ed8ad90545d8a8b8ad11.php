              ...
              <div class="card">
                <div class="card-head">
                  <div class="card-header">
                    <h4 class="card-title">  <?php echo e(__('backend.assessment')); ?>   </h4>
...
<form method="post" action="<?php echo e(url('')); ?>/<?php echo e(config('settings.BackendPath')); ?>/post_assessment_update_products_delegate/<?php echo e($customer->id); ?>">
...
<table  class="table table-striped table-bordered zero-configuration dataTable">
  <thead>
    <tr>
      ...
      <th> <?php echo e(__('backend.by')); ?>  </th>
  ...
  <?php $__currentLoopData = $total_products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <tr>
      ...
      <th> <?php if($product->request_by == 'delegates'): ?> <?php echo e('تقييم'); ?> <?php else: ?> <?php echo e('احالة'); ?> <?php endif; ?>  </th>
      <td ><?php echo Form::select('unit_id[]', $units ,  $product->unit_id , ['id'=>'unit_id_'.$product->id, 'class' => 'form-control' , 'style'=>'width:100px;'] ); ?></td>
      <td><?php echo Form::number('quantity[]', $product->quantity , ['id'=>'quantity' , 'product_id'=> $product->id , 'class' => 'form-control' , 'style'=>'width:80px;' , 'placeholder'=> __('backend.quantity') ] ); ?></td>
      <td><?php echo Form::text('price[]', $product->price , ['id'=>'price_'.$product->id, 'class' => 'form-control', 'style'=>'width:80px;'  ,  'placeholder'=> __('backend.price') ] ); ?></td>
      <td><?php echo Form::text('estimate_consumption[]',  $product->estimate_consumption , ['id'=>'estimate_consumption_'.$product->id , 'style'=>'width:80px;', 'class' => 'form-control' , 'placeholder'=> __('backend.estimate_consumption') ] ); ?></td>

      <th> <a href="<?php echo e(url('')); ?>/<?php echo e(config('settings.BackendPath')); ?>/assessment_products/<?php echo e($product->id); ?>/delete" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>  </th>
      <th> <?php echo e($product->created_at); ?>  </th>
    </tr>
    <?php echo Form::hidden('products_doc[]', $product->product_id  , ['id'=>'products_doc'.$product->id, 'class' => 'form-control'  ] ); ?>

    <?php echo Form::hidden('assessment_id[]', $product->id  , ['id'=>'assessment_id'.$product->id, 'class' => 'form-control'  ] ); ?>
