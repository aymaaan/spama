              ...
              <div class="card">
                <div class="card-head">
                  <div class="card-header">
                    <h4 class="card-title"> {{ __('backend.categories') }} </h4>
...
{!! Form::model( $data ,[ 'url' =>  config('settings.BackendPath').'/categories/'.$data->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}  

@include('backend.pages.categories.form')
...
@section('head')
        ...
        <title>{{ __('backend.categories') }} | {{config('settings.sitename')}}</title>
