...
<div class="app-content content">
  <div class="content-body">
    <section class="row">
        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> <B>   {{ __('backend.age_categories') }}    </B></h4>
              <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a>
              @can('create_age_categories')
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/age_categories/create">


                  <button class="btn btn-primary btn">
                    <i class="la la-plus-square-o"></i> {{ __('backend.add_age_categories') }}</button>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @can('delete_age_categories') 
                          <a title="Delete Post" href="{{url(config('settings.BackendPath').'/age_categories/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan



                          @can('update_age_categories') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/age_categories/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan
      ...
      <title> {{ __('backend.age_categories') }} | {{config('settings.sitename')}}</title>
