...
<div class="app-content content">
  <div class="content-body">
    <section class="row">
        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> <B>   {{ __('backend.brands') }}    </B></h4>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @can('delete_brands') 
                          <a title="Delete Post" href="{{url(config('settings.BackendPath').'/brands/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan
                          ...
                          @can('update_brands') 
                          @if($row->status == 1 )
                          <a title='Block brands' href="{{url(config('settings.BackendPath').'/brands/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish brands' href="{{url(config('settings.BackendPath').'/brands/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan


                          @can('update_brands') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/brands/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan
      ...
      <title> {{ __('backend.brands') }}  | {{config('settings.sitename')}}</title>
