...
<div class="app-content content">
  <div class="content-body">
    <section class="row">
        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> <B>   {{__('backend.websites')}}     </B></h4>
              <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a>
              @can('create_websites')
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/websites/create">


                  <button class="btn btn-primary btn">
                    <i class="la la-plus-square-o"></i>    {{__('backend.add_websites')}}    </button>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @can('delete_websites') 
                          <a title="Delete Post" href="{{url(config('settings.BackendPath').'/websites/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan
                          ...
                          @can('update_websites') 
                          @if($row->status == 1 )
                          <a title='Block websites' href="{{url(config('settings.BackendPath').'/websites/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish websites' href="{{url(config('settings.BackendPath').'/websites/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan


                          @can('update_websites') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/websites/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan
      ...
      <title> {{__('backend.websites')}} | {{config('settings.sitename')}}</title>
