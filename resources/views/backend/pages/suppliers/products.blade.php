        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> <B>   {{ __('backend.products') }} |  {{$data->name}}   </B></h4>
                    ...
                    <thead>
                      <tr>
                        ...
                        <th>  {{ __('backend.category') }}  </th>
                          <th> {{ __('backend.customer_price') }}    </th>
...
@foreach ( $data->products as $row)    
                      ...
                      <tr>
<td><a target="_blank" href="{{url(config('settings.BackendPath'))}}/product/{{$row->id}}">{{$row->sku}}</a></td>
<td>{{$row->title_ar}}</td>
<td style="text-align:left;" >{{$row->title_en}}</td>
<td>{{$row->category['title']}}</td>
<td>@if(isset($row->unit_prices[0]['customer_price'])) {{$row->unit_prices[0]['customer_price']}} @endif</td>
