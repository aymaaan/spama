          ...
          <div class="card-head">
            <div class="card-header">
              ...
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/sub-features-products/create?cat={{$_GET['cat']}}">
...
<select  class="form-control" onchange="if( this.options[this.selectedIndex].value != '' ) location.href=this.options[this.selectedIndex].value;">
  ...
  <option  value="{{url(config('settings.BackendPath').'/sub-features-products/order_list/'. $row->id.'/'.$i)}}">{{$i}}</option>
  ...
  @endfor
                        ...
                        <td>
                          ...
                          <a title="Delete Post" href="{{url(config('settings.BackendPath').'/sub-features-products/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan
                          ...
                          @if($row->status == 1 )
                          <a title='Block features' href="{{url(config('settings.BackendPath').'/sub-features-products/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish features' href="{{url(config('settings.BackendPath').'/sub-features-products/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan
                           ...
                           @can('update_features_products') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/sub-features-products/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan
