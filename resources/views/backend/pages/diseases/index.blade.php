...
<div class="app-content content">
  <div class="content-body">
    <section class="row">
        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> <B>   {{ __('backend.diseases') }}    </B></h4>
              <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a>
              @can('create_diseases')
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/diseases/create">


                  <button class="btn btn-primary btn">
                    <i class="la la-plus-square-o"></i> {{ __('backend.add_diseases') }}</button>
                  </a> </div> 
              @endcan
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @can('delete_diseases') 
                          <a title="Delete diseases" href="{{url(config('settings.BackendPath').'/diseases/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan

                          @can('update_diseases') 
                          @if($row->status == 1 )
                          <a title='Block diseases' href="{{url(config('settings.BackendPath').'/diseases/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish diseases' href="{{url(config('settings.BackendPath').'/diseases/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan

                          @can('update_diseases') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/diseases/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                          @endcan
      ...
      <title> {{ __('backend.diseases') }} | {{config('settings.sitename')}}</title>
