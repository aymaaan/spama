...
<div class="app-content content">
  <div class="content-body">
    <section class="row">
        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> {{ __('backend.mother_products') }} | <B>
               @if( $AppLangUser == 'ar' )
               {{$category->title}} 
               @else
               {{$category->title_en}}
              ...
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/mother-products/create?cat={{$_GET['cat']}}">
                  <button class="btn btn-primary btn">
                    <i class="la la-plus-square-o"></i>   {{ __('backend.add_mother_product') }}    </button>
                  </a> </div> 
              @endcan
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <a title="Delete Post" href="{{url(config('settings.BackendPath').'/mother-products/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan
                          ...
                          @if($row->status == 1 )
                          <a title='Block categories' href="{{url(config('settings.BackendPath').'/mother-products/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish categories' href="{{url(config('settings.BackendPath').'/mother-products/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan
                           ...
                           @can('update_mother_products') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/mother-products/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan


                   @can('names_products') 

                  <a href="{{url('')}}/{{config('settings.BackendPath')}}/names-products?cat={{$row->id}}">
                  <button class="btn btn-success btn">
                  {{ __('backend.names') }}   <span class="badge badge badge-dark  badge-pill float-right mr-2"> {{ count( $row->names )}}  </span></button>
                  </a> 
      ...
      <title>{{ __('backend.mother_products') }} | {{config('settings.sitename')}}</title>
