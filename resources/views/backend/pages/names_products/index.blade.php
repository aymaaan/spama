...
<div class="app-content content">
  <div class="content-body">
    <section class="row">
        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title">    {{ __('backend.mother_products') }}    
               ...
               @endif
                ...
                | <B>  {{ __('backend.names') }}  <B></h4>
              <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a>
              @can('create_mother_products')
              @if( $data->count() < 9 )
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/names-products/create?cat={{$_GET['cat']}}">
                  <button class="btn btn-primary btn">
                    <i class="la la-plus-square-o"></i>      {{ __('backend.add_product_name') }}      </button>
                  </a> </div> 
                  @endif
              @endcan
              ...
              <div class="card-content">
                <div class="card-body">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @can('delete_mother_products') 
                          <a title="Delete Post" href="{{url(config('settings.BackendPath').'/names-products/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan



                          @can('update_mother_products') 
                          @if($row->status == 1 )
                          <a title='Block categories' href="{{url(config('settings.BackendPath').'/names-products/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish categories' href="{{url(config('settings.BackendPath').'/names-products/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan
                           ...
                           @can('update_mother_products') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/names-products/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan
                  ...
                  <a href="{{url('')}}/{{config('settings.BackendPath')}}/features-products?cat={{$row->id}}">
                  <button class="btn btn-success btn">
                  {{ __('backend.features') }}   <span class="badge badge badge-dark  badge-pill float-right mr-2"> {{ count( $row->features )}}  </span></button>
                  </a> 
      ...
      <title> {{ __('backend.names') }} | {{config('settings.sitename')}}</title>
