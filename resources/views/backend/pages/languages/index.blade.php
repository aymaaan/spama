...
<div class="app-content content">
  <div class="content-body">
    <section class="row">
        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> <B>   {{ __('backend.languages') }}    </B></h4>
              <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a>
              @can('create_languages')
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/languages/create">

                  <button class="btn btn-primary btn">
                    <i class="la la-plus-square-o"></i>   {{ __('backend.add_language') }}      </button>
                  ...
                  <table class="table table-striped table-bordered default-ordering">
                    <thead>
                      <tr>
                        ...
                        <th> {{ __('backend.translate_words') }}  </th>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                        <a class="btn btn-success btn" href="{{url('')}}/{{config('settings.BackendPath')}}/languages/words/{{$row->code}}"><i class="la la-plus-square-o"></i>   {{ __('backend.translate_words') }}</a></td>

                        <td>


                          @can('delete_languages') 
                          <a title="Delete languages" href="{{url(config('settings.BackendPath').'/languages/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan
                          ...
                          @if($row->status == 1 )
                          <a title='Block languages' href="{{url(config('settings.BackendPath').'/languages/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish languages' href="{{url(config('settings.BackendPath').'/languages/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan


                          @can('update_languages') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/languages/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan
      ...
      <title> {{ __('backend.languages') }}  | {{config('settings.sitename')}}</title>
