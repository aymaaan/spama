...
<div class="app-content content">
  <div class="content-body">
    <section class="row">
        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> <B>   {{__('backend.sales_channels')}}     </B></h4>
              <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a>
              @can('create_sales_channels')
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/sales_channels/create">


                  <button class="btn btn-primary btn">
                    <i class="la la-plus-square-o"></i>    {{__('backend.add_sales_channels')}}    </button>
                  ...
                  <table id="users-contacts" style='width:100%;' class="table datatable table-hover table-responsive">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{$row->title}}</td>
                        <td>{{$row->title_en}}</td>
                        <td class="badge badge badge-success float-right">{{count($row->delegates)}}</td>
 
                        <td>


                          @can('delete_sales_channels') 
                          <a title="Delete Post" href="{{url(config('settings.BackendPath').'/sales_channels/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan



                          @can('update_sales_channels') 
                          @if($row->status == 1 )
                          <a title='Block sales_channels' href="{{url(config('settings.BackendPath').'/sales_channels/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish sales_channels' href="{{url(config('settings.BackendPath').'/sales_channels/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan


                          @can('update_sales_channels') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/sales_channels/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan
      ...
      <title> {{__('backend.sales_channels')}} | {{config('settings.sitename')}}</title>
