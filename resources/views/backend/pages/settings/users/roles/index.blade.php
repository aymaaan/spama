              ...
              <div class="card">
                <div class="card-head">
                  <div class="card-header">
                    <h4 class="card-title"> {{ __('backend.roles')}} </h4>
                    <a class="heading-elements-toggle"><i class="la la-ellipsis-h font-medium-3"></i></a> @can('create_role')
                    <div class="heading-elements">
                    <a href="{{url('')}}/{{config('settings.BackendPath')}}/roles/create">
                      <button class="btn btn-primary btn">
                      <i class="la la-plus-square-o"></i>   {{ __('backend.add_role')}}  </button>
                      </a>
...
@foreach ( $roles as $role )
               ...
               <td>
                ...
                @foreach ($role->permissions->where('parent_id' , 0) as $permission )
                
               <p> عرض  {{$permission->label}} </p>
               

                @foreach ($permission->list_permissions as $list_permission )

               <li> {{$list_permission->label}} </li>
...
<hr>
               ...
               <td> @can('update_role')
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/roles/{{$role->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a> @endcan
                </td>
