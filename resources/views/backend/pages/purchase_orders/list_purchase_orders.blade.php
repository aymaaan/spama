        ...
        <div class="card">
          <div class="card-head">
            <div class="card-header">
              <h4 class="card-title"> <B>   {{ __('backend.list_purchase_orders') }}    </B></h4>
...
@if( $product->pivot->quantity_each_repository <= $product->pivot->minimum_quantity_each_repository)
...
<h4 class="form-section"><i class="la la-commenting"></i>    {{$row->title_ar}}   </h4>
                  ...
                  <table id="users-contacts" style='width:100%;' class="table datatable table-hover table-responsive">
                    <thead>
                      <tr>
                        ...
                        <th> {{ __('backend.minimum_total_quantity') }}  </th>
                        <th> {{ __('backend.maximum_quantity_each_repository') }}  </th>
                        <th> كمية امر الشراء  </th>
                        <th> {{ __('backend.suppliers') }}  </th>
                      </tr>
                    </thead>
                    <tbody>
                     ...
                     @if( $product->pivot->quantity_each_repository <= $product->pivot->minimum_quantity_each_repository)
                      ...
                      <tr>
                        ...
                        @endif
                        <th> {{$product->pivot->quantity_each_repository}}  </th>
                        <th> {{$product->pivot->minimum_quantity_each_repository}}  </th>
                        <th> {{$product->pivot->maximum_quantity_each_repository}}  </th>

                        <th> {{$product->pivot->maximum_quantity_each_repository - $product->pivot->quantity_each_repository }}  </th>

                        <th>
@if(count($product->suppliers->where('supplier_type' , 'داخلى')))
مورد داخلى : </br>
...
@if(count($product->suppliers->where('supplier_type' , 'خارجى')))
...
@endforeach
      ...
      <title> {{ __('backend.add_purchase_orders') }} | {{config('settings.sitename')}}</title>
