              ...
              <div class="heading-elements">
                <a href="{{url('')}}/{{config('settings.BackendPath')}}/orders/create">
                  ...
                  <button class="btn btn-primary btn">
                    <i class="la la-plus-square-o"></i>    {{__('backend.add_orders')}}    </button>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @can('delete_orders') 
                          <a title="Delete Post" href="{{url(config('settings.BackendPath').'/orders/'.$row->id.'/delete')}}" class="badge badge badge-danger float-right"><i class="la la-trash"></i> </a>
                          @endcan
                          ...
                          @can('update_orders') 
                          @if($row->status == 1 )
                          <a title='Block orders' href="{{url(config('settings.BackendPath').'/orders/approve/'.$row->id.'/0')}}" class="badge badge badge-success float-right"><i class="la la-check"></i> </a>
                          @else
                          <a title='Publish orders' href="{{url(config('settings.BackendPath').'/orders/approve/'.$row->id.'/1')}}" class="badge badge badge-danger float-right"><i class="la la-ban "></i> </a>
                          @endif
                          @endcan


                          @can('update_orders') 
                              <a href="{{url('')}}/{{config('settings.BackendPath')}}/orders/{{$row->id}}/edit" class="badge badge badge-info float-right"><i class="la la-pencil"></i> </a>
                           @endcan
